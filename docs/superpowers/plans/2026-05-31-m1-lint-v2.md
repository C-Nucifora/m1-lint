...
> **Note:** All example identifiers/snippets below are synthetic placeholders, not from any real project. Corpus tests resolve their path from `M1_CORPUS_PATH` (falling back to the sibling `EV-M1` project) and are skipped when unset.
